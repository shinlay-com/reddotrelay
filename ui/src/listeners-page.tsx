...
type Health = "synced" | "lagging" | "paused" | "idle" | "failed";
  ...
  return <section class="listeners-redesign">
    ...
    <div class="listener-master-detail">
      ...
      {selected ? <article class="panel listener-detail">
        <header><div><span>{selected.name.slice(0, 1).toUpperCase()}</span><div><div><h2>{selected.name}</h2><b class={`progress-status progress-status--${selectedHealth}`}>{selectedHealth}</b></div><p>Chain {selected.chainId} · {selectedHealth === "synced" ? "Scanner is caught up" : selectedHealth === "lagging" ? "Scanner is behind the confirmed head" : selectedHealth === "paused" ? "Listener is disabled by configuration" : selectedHealth === "idle" ? "No event subscriptions configured" : "Listener needs attention"}</p>{selectedHealth === "failed" && selectedFailure && <p class="connection-error">{selectedFailure}{selectedProgress?.lastErrorAt && <small>{new Date(selectedProgress.lastErrorAt).toLocaleString()}</small>}</p>}</div></div></header>
