...
type ScannerProgress = { rpcListenerId: string; chainId: number; latestBlock: number; confirmedHead: number; checkpoint: number; lagBlocks: number; updatedAt: string; name?: string; state?: ProgressState; available?: boolean };
...
export function DeliveryOperations({ session, listeners, runtimeListeners }: { session: Session; listeners: RPCListener[]; runtimeListeners: RuntimeListener[] }) {
  ...
  const [filters, setFilters] = useState({ transactionHash: "", blockNumber: "", contractAddress: "", eventSignature: "", deliveryStatus: "" });
  ...
  const displayedProgress = listeners.map((listener) => {
    ...
    if (!item) return {
      ...
      state: (listener.paused ? "paused" : "failed") as ProgressState, available: false
    };
    return {
    ...item,
      name: listenerByID.get(item.rpcListenerId)?.name ?? "RPC listener",
      state: (listener.paused ? "paused" : item.lagBlocks > 0 ? "lagging" : runtime?.state === "running" ? "synced" : "failed") as ProgressState,
      available: true
    };
  });

  async function loadEvents(targetCursor?: string) {
    ...
    try {
      ...
      const page = await read<EventPage>(`/api/v1/events?${query}`); setEvents(page.entries); setNextBefore(page.nextBefore);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Event history is unavailable."); } finally { setBusy(false); }
  }
  ...
  useEffect(() => { setEventCursor(undefined); setEventHistory([]); setExpanded(undefined); void loadEvents(); void loadProgress(); }, [pageSize]);
  ...
  function update(name: keyof typeof filters, value: string) { setFilters({ ...filters, [name]: value }); }
  function clearFilters() { setFilters({ transactionHash: "", blockNumber: "", contractAddress: "", eventSignature: "", deliveryStatus: "" }); }
  ...
  return <section class="delivery-operations"><div class="section-title"><div><p class="eyebrow">Durable operations</p><h1>Events and deliveries</h1></div><button class="button button--secondary" type="button" disabled={busy} onClick={() => void refresh()}>{busy ? "Refreshing…" : "Refresh"}</button></div>
    {displayedProgress.length > 0 && <div class="scanner-progress">{displayedProgress.map((item) => <article class={`panel scanner-progress--${item.state}`} key={item.rpcListenerId}><div><strong>{item.name}</strong><span>Chain {item.chainId}</span><span class={`progress-status progress-status--${item.state}`}>{item.state}</span></div><dl><div><dt>Latest</dt><dd>{item.available ? item.latestBlock.toLocaleString() : "—"}</dd></div><div><dt>Confirmed</dt><dd>{item.available ? item.confirmedHead.toLocaleString() : "—"}</dd></div><div><dt>Checkpoint</dt><dd>{item.available ? item.checkpoint.toLocaleString() : "—"}</dd></div><div><dt>Lag</dt><dd>{item.available ? `${item.lagBlocks.toLocaleString()} blocks` : "—"}</dd></div></dl></article>)}</div>}
    <form class="panel diagnostic-filters" onSubmit={(event) => { event.preventDefault(); setExpanded(undefined); setEventCursor(undefined); setEventHistory([]); void loadEvents(); }}>
      ...
      <div class="diagnostic-filter-actions"><button class="button" disabled={busy}>Search</button><button class="button button--secondary" type="button" disabled={busy || !hasFilters} onClick={clearFilters}>Clear all</button></div>
    </form>
